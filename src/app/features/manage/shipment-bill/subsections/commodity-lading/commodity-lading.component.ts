...
import { IBillSection, ICommodityLadingFG } from '@otc/features/manage/shipment-bill/models/shipment-bill.interface';
...
@Component({
  ...
  templateUrl: './commodity-lading.component.html',
  styleUrl: './commodity-lading.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CommodityLadingComponent implements OnInit, OnDestroy {
  protected readonly sectionLabel: typeof EBillSectionLabel = EBillSectionLabel;
  form: InputSignal<FormGroup<ICommodityLadingFG>> = input.required<FormGroup<ICommodityLadingFG>>();
  ...
  onExpandPanel($event: boolean): void {
    this.expanded.emit({ key: EBillSection.COMMODITY_LADING, expanded: $event.valueOf() } as IBillSection);
  }
}
